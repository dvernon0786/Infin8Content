-- Migration: Add approval tracking fields for human-in-the-loop workflow
-- Date: 2026-02-17
-- Purpose: Enable entity-level approval gating for Steps 3-9

-- Add approval tracking to keywords table (seeds and longtails)
ALTER TABLE keywords 
ADD COLUMN user_selected BOOLEAN DEFAULT false,
ADD COLUMN selection_source VARCHAR(10) DEFAULT 'ai',
ADD COLUMN selection_updated_at TIMESTAMP;

-- Add approval tracking to topic_clusters table
ALTER TABLE topic_clusters 
ADD COLUMN user_selected BOOLEAN DEFAULT false,
ADD COLUMN selection_source VARCHAR(10) DEFAULT 'ai',
ADD COLUMN selection_updated_at TIMESTAMP;

-- Add approval tracking to subtopics table  
...
CREATE INDEX idx_keywords_user_selected ON keywords(user_selected, workflow_id);
CREATE INDEX idx_keywords_approval ON keywords(user_selected, organization_id, workflow_id);
CREATE INDEX idx_clusters_user_selected ON topic_clusters(user_selected, workflow_id);
CREATE INDEX idx_clusters_approval ON topic_clusters(user_selected, organization_id, workflow_id);
CREATE INDEX idx_subtopics_user_selected ON subtopics(user_selected, workflow_id);
CREATE INDEX idx_subtopics_approval ON subtopics(user_selected, organization_id, workflow_id);
