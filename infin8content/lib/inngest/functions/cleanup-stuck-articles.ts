...
/**
 * Cleanup job to detect and fix articles stuck in "processing" status
 * Runs every 30 minutes to catch articles that timed out without proper error handling
 * 
 * This prevents articles from being stuck indefinitely in "processing" status
...
export const cleanupStuckArticles = inngest.createFunction(
    ...
    async ({ step }) => {
        ...
        return await step.run('find-and-fix-stuck-articles', async () => {
            // Find articles stuck in "processing" for more than 30 minutes
            ...
            const { data: stuckArticlesData, error: findError } = await (supabase
                ...
                .select('id, keyword, updated_at, current_section_index, sections')
                .eq('status', 'processing')
                .lt('updated_at', thirtyMinutesAgo) as unknown as Promise<{
                    data: Array<{ id: string; keyword: string; updated_at: string; current_section_index?: number; sections?: any[] }> | null;
                    error: any
                }>)
            ...
            const { error: updateError, data: updatedArticles } = await supabase
                ...
                .update({
                    status: 'failed',
                    error_details: {
                        error_message: 'Article generation timed out (stuck in processing status for >30 minutes)',
                        failed_at: new Date().toISOString(),
                        timeout: true,
                        auto_cleaned: true,
                        stuck_duration_minutes: stuckArticles.map(article => {
                            const startTime = new Date(article.updated_at).getTime()
                            const now = Date.now()
