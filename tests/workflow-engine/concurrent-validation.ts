/**
 * Direct Concurrent Validation Test
 * Tests the workflow engine at the database level using service role client
 * This bypasses the API authentication layer and tests the core atomicity
 */
...
async function testAtomicity() {
  ...
  await supabase
    ...
    .update({ state: WorkflowState.COMPETITOR_PENDING })
    .eq('id', WORKFLOW_ID)
  ...
  const passed = successes === 1 && failures === 2 && workflow?.state === WorkflowState.COMPETITOR_PENDING

  console.log(passed ? '✅ PASS: Atomicity verified' : '❌ FAIL: Race condition detected')
  return passed
}
 ...
 */
async function attemptTransition(requestId: number): Promise<{ success: boolean }> {
  const { data, error } = await supabase
    .from('intent_workflows')
    .update({
      state: WorkflowState.COMPETITOR_PROCESSING,
      updated_at: new Date().toISOString(),
    })
    .eq('id', WORKFLOW_ID)
    .eq('organization_id', ORG_ID)
    .eq('state', WorkflowState.COMPETITOR_PENDING)
    .select('id, state')
    .single()
...
async function testStatePurity() {
  ...
  const { data: processing } = await supabase
    .from('intent_workflows')
    .update({ state: WorkflowState.COMPETITOR_PROCESSING })
    .eq('id', WORKFLOW_ID)
    .eq('organization_id', ORG_ID)
    .select('state')
    .single()
  ...
  const { data: completed } = await supabase
    .from('intent_workflows')
    .update({ state: WorkflowState.COMPETITOR_COMPLETED })
    .eq('id', WORKFLOW_ID)
    .eq('organization_id', ORG_ID)
    .select('state')
    .single()
  ...
  const passed =
    processing?.state === WorkflowState.COMPETITOR_PROCESSING &&
    completed?.state === WorkflowState.COMPETITOR_COMPLETED
...
async function testConcurrencySafety() {
  ...
  const passed = successes === 1 && failures === 19 && workflow?.state === WorkflowState.COMPETITOR_PROCESSING
