...
* **Epic 34.1** (ICP Generation) - COMPLETED ✅ - Provides organization context and competitor URLs
* **Story 34.3** (Longtail Expansion) - Downstream consumer of seed keywords
* **Story 34.4** (Subtopic Generation) - Downstream consumer of seed keywords

### Key Integration Points

1. **Workflow State Machine**: Uses intent_workflows table status field
2. **Organization Context**: Requires organization_id from current user
3. **Competitor Data**: Reads from competitors table (populated by Epic 34.1)
4. **Keyword Pipeline**: Writes to keywords table (normalized, not JSON)

### Development Guardrails

* ✅ No UI event emission (backend-only step)
* ✅ Terminal state analytics only (workflow completion)
* ✅ Producer-only responsibility (creates keywords, doesn't consume)
* ✅ Idempotent execution (safe to re-run)
* ✅ Non-blocking per competitor (continues on partial failure)
