-- Migration: Create atomic cost guard functions (after PL/pgSQL enabled)
-- Date: 2026-02-12
-- Purpose: Prevent race conditions in AI cost enforcement
-- Run this after enabling PL/pgSQL extension

-- Drop functions if they exist (to recreate cleanly)
DROP FUNCTION IF EXISTS check_and_update_workflow_cost(uuid, numeric, numeric);
DROP FUNCTION IF EXISTS get_organization_monthly_ai_cost(uuid);
DROP FUNCTION IF EXISTS check_organization_monthly_quota(uuid, numeric);
